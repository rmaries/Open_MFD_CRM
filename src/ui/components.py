...
def input_client_details(db):
    ...
    with st.form("onboarding_form", clear_on_submit=True):
        name = st.text_input("Full Name (as per PAN)")
        pan = st.text_input("PAN (10 characters)").upper()
        ...
        if submitted:
            if not name or not pan:
                st.error("Name and PAN are required!")
            else:
                try:
                    client_id = db.add_client(name, pan, can_number=can, email=email, phone=phone)
